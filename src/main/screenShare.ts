...
export function registerScreenShareHandler() {
    ...
    session.defaultSession.setDisplayMediaRequestHandler(async (request, callback) => {
        ...
        if (isWayland) {
            ...
            if (video) {
                const stream = await request.frame
                    .executeJavaScript(
                        `Resktop.Components.ScreenShare.openScreenSharePicker(${JSON.stringify([video])},true)`
                    )
                    .catch(() => null);
        ...
        const choice = await request.frame
            .executeJavaScript(`Resktop.Components.ScreenShare.openScreenSharePicker(${JSON.stringify(data)})`)
