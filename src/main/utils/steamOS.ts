...
const layoutId = "3080264545"; // Resktop Layout v2
...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Resktop Steam Input Layout?`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Resktop's recommended Steam Deck controller settings?
